...
class Leveler(commands.Cog):
    ...
    def make_full_profile(self, avatar_data, user, xp, nxp, lvl, minone, elo, ldb, desc, bg=None):
        img = Image.new("RGBA", (480, 120), (0, 0, 0, 255))
        if bg is not None:
        ...
        barcolor = (64, 114, 148)
        usercolor = (255,255,255)  # user.color.to_rgb()
        grey = (205, 205, 205)

        xptot = self.add_corners(Image.new("RGBA", (310, 20), (17, 17, 17, 255)), 10)
        img.paste(xptot, (140, 80), xptot)
        ...
        fontpath = str(bundled_data_path(self) / "Helvetica.otf")
        
        font1 = ImageFont.truetype(fontpath, 18)
        font2 = ImageFont.truetype(fontpath, 22)
        font3 = ImageFont.truetype(fontpath, 28)
        ...
        # draw.text((5,5), f"{b_offset}", fill="yellow", font=font3)
        # draw.text((100,5), f"{minone}", fill="yellow", font=font3)
        xpbar = self.add_corners(Image.new("RGBA", (b_offset, 20), barcolor), 10)
        img.paste(xpbar, (140, 80), xpbar)
        ...
        draw.text((140, 50), lvl_str, fill=grey, font=font1)
        draw.text((320, 50), ldb_str, fill=grey, font=font1)        
        draw.text((245, 98), (f"{xp}/{nxp} XP"), fill=grey, font=font1)   
        draw.text((200, 47), f"{lvl}", fill=barcolor, font=font2)
        draw.text((400, 47), f"{ldb}", fill=barcolor, font=font2)
        draw.text((140, 10), f"{user.name}#{user.discriminator}", fill=usercolor, font=font3)
        temp = BytesIO()
    ...
    async def reg_all(self, ctx):
        for member in ctx.guild.members:
            ...
            if await self.profiles._is_registered(member):
                ...
                await self.profiles._register_user(member)
                # await ctx.send(_("You have been successfully registered !"))
        await ctx.send(_("All users have been successfully registered!"))
    ...
    async def setlevel(self, ctx, level: int, member: discord.Member = None):
        ...
        if await self.profiles._is_registered(member):
            await self.profiles._set_exp(member, 100 * 2 ** (level - 1))
        else:
